...
from dc.utils.fmt import fmt
from dc.utils.shell import cd, call, rglob, globs, which
...
def pyfiles():
    with cd(REPOROOT):
        return sorted(glob('dc/**/*.py', recursive=True))
...
def task_pycov():
    ...
    return dict(
        actions=[
            fmt('pytest {PYTHON} -m pytest -s -vv --cov={DCDIR} {TESTDIR}'),
        ],
    )
